// komari-agent-rs: FreeBSD IP detection — getifaddrs FFI + external HTTP APIs.
...
// FreeBSD: AF_INET=2, AF_INET6=28 (not 30 as on Darwin)
const AF_INET: u8 = 2;
...
pub fn collect_ip(config: &Config) -> Result<(Option<String>, Option<String>), MetricErr> {
    ...
    if config.get_ip_addr_from_nic {
        let allow_nics = nic_list(&config.include_nics, &config.exclude_nics);
        let (v4, v6) = ip_from_interfaces(&allow_nics);
...
// ═══════════════════════════════════════════════════════════════════
// NIC listing via getifaddrs (matching Go InterfaceList behavior)
// ═══════════════════════════════════════════════════════════════════

fn nic_list(include: &[String], exclude: &[String]) -> Vec<String> {
    let mut out = Vec::new();
    let mut ifa_head: *mut IfAddrs = std::ptr::null_mut();
    if unsafe { getifaddrs(&mut ifa_head) } != 0 {
        return out;
    }
...
/// Detect IPs from a list of interface names via getifaddrs.
fn ip_from_interfaces(nic_names: &[String]) -> (Option<String>, Option<String>) {
    let mut ifa_head: *mut IfAddrs = std::ptr::null_mut();
    ...
    while !cur.is_null() {
        ...
        // Filter: skip if NIC not in allow list
        if !nic_names.iter().any(|n| n == name) {
            cur = ifa.ifa_next;
            continue;
        }
        ...
        match family {
            AF_INET if ipv4.is_none() => {
                ...
                let addr_ptr = unsafe { ifa.ifa_addr as *const u8 };
                let a = unsafe { std::slice::from_raw_parts(addr_ptr.add(4), 4) };
                ipv4 = Some(format!("{}.{}.{}.{}", a[0], a[1], a[2], a[3]));
            }
            AF_INET6 if ipv6.is_none() => {
                // sin6_addr starts at offset 8 in struct sockaddr_in6 (BSD layout)
                let addr_ptr = unsafe { ifa.ifa_addr as *const u8 };
                let a = unsafe { std::slice::from_raw_parts(addr_ptr.add(8), 16) };
