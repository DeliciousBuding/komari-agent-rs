...
pub fn make_tls_config(config: &Config) -> Result<rustls::ClientConfig, TlsErr> {
    ...
    if config.ignore_unsafe_cert {
        ...
        // is an HTTP/1.1 feature — an nginx reverse proxy fronting a Komari
        // deployment often defaults to HTTP/2 and silently drops the `Upgrade`
        // header over h2, returning the SPA index.html (200) instead of 101
        // Switching Protocols. Pinning ALPN to h1 makes the WS handshake
        // succeed (parity with Go's gorilla/websocket, which also negotiates
        // http/1.1).
